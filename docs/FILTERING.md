...
- Viewport-first and **concurrent** (up to 6 parallel fetches — the browser's natural per-host limit, indistinguishable from a user opening several listings in tabs), so a full page is vetted in a couple of seconds rather than the ~25s a serial crawl took. Cards on screen are fetched before the look-ahead ring, so the listing you're about to click resolves first. Safety comes from a **rolling-window rate cap** (bounded fetches per minute) plus **adaptive backoff** — a 429/503 or a run of errors pauses fetching entirely — not from crawling slowly; paused while the tab is hidden. The pill shows monotonic progress ("checking 8/24") that only fills forward, the page keeps re-checking until verdicts land, and opening a listing scores it instantly regardless.
