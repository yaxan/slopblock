...
- Viewport-first and throttled (one fetch at a time with jittered ~1.1s spacing, hard per-page cap, error backoff, paused while the tab is hidden) so request volume stays proportional to what the user actually looks at. Cards on screen are fetched before the look-ahead ring, so the listing you're about to click resolves first. **This means a verdict can land a second or two after a card appears** — the pill shows "· scanning N" while fetches are outstanding, the page keeps re-checking until they land, and opening a listing scores it instantly regardless.
