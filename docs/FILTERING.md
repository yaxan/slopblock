...
1. **The default is gem hunting.** SlopBlock ships tuned for finding quality secondhand pieces: **anything mentioning IKEA is hidden outright** (flat-pack is noise for gem hunters, used or new — one tap switches it to Filter or Off), and **brand-new/in-box titles dim** ("Brandnew", "New, in Box |", "never used" — retail flips, not gems; bare "sealed" is exempt so collectors keep their LEGO). Both are instant card-text verdicts with zero network latency, both are tri-state quick filters, and both are gate-tested in BOTH directions: the default profile must action them, and a permissive profile must show every legit listing untouched.
2. **Vendor mentions are context, not crimes (in Filter mode).** "Amazon Echo", "bought from Wayfair originally" are everyday legit resales. Vendor rules sit in the low bands, and the `vendor-retail-combo` rule escalates only when a vendor mention appears alongside retail-style wording with no human condition details. Amazon product names (Echo, Kindle, Fire, Basics…) are excluded from the Amazon source rule entirely.
2b. **Retail platform artifacts are near-certain slop.** Shop-catalog product cards ("View in 3D", "Choose color:" configurators) are retailers advertising inside search results, not people selling things — they hide on sight, and card scanning harvests aria-labels so icon-only overlays are seen too. Tokenization variants count too ("Brandnew" = "brand new"), and "read description" on a Free/$1 placeholder price is bait even without the word "price".
3. **Everyday phrasing is not evidence.** "$15 each", "cash or venmo", "delivery available", "text me at 416-…", "brand new in box — unwanted gift", "no dealer fees here" (negation-aware), and price-drop strikethroughs are all normal human listing language and are specifically kept out of the strong rules.
4. **Definitive signals are not rescued by condition words.** "Good condition" doesn't make a SOLD post buyable, so gem-positive offsets are ignored once a 70+ signal fires. User allowlists still always win.
5. **Hide requires certainty (for spam verdicts).** Taste defaults (IKEA, brand-new) are explicit product choices, not evidence claims — which is why they live in reversible quick filters rather than the weights. Single medium-confidence rules can at most dim. Hiding requires either one near-certain signal or several independent ones.
...
2b. **The popup notices what keeps getting past you.** When a vendor-style filter in Filter mode matches 2+ listings still on screen (visible or merely dimmed), the summary shows "Seeing too much?" with a one-tap **Hide all** — the escalation lever appears exactly where the annoyance happens (e.g. brand-new/in-box items, Amazon/Temu mentions; IKEA already ships in Hide all).
3. **Popup → Quick filters**: the opinionated areas in one place. Vendor-flavored filters (Amazon/Temu sources, IKEA, brand-new/in-box items, liquidation) are three-way — **Off** (ignore entirely), **Filter** (treat as slop evidence), **Hide all** (remove every match — an explicit personal block, weight 96, still rescuable by allowlists). Shipped defaults: IKEA = Hide all (gem hunting), brand-new/in-box = Filter (dims), Amazon/Temu and liquidation = Filter. The rest are on/off: sponsored ads, catalog copy, repeated listings, not-for-sale posts, job/money pitches, vague commercial text.
